...
const client = new ApolloClient({
  uri: `${process.env.NEXT_PUBLIC_ORDER_HISTORY_URL}/graphql`,
...
const AnalyticsReport = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const historicalDataResult = await client.query({
        query: HISTORICAL_DATA_QUERY,
      });
      const companyInfoResult = await client.query({
        query: COMPANY_INFO_QUERY,
      });
      const userPortfolioResult = await client.query({
